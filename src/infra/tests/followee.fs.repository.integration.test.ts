...
import {FileSystemFolloweeFsRepository} from "../followee.fs.repository";
...
describe("FileSystemFolloweeRepository", () => {
    ...
    test("saveFollowee() should save a new followee", async () => {
        const followeeRepository
            = new FileSystemFolloweeFsRepository(testFolloweesPath);
        await fs.promises.writeFile(testFolloweesPath,JSON.stringify({
            Alice: ["Bob"],
            Bob: ["Charlie"]
        }))
        await followeeRepository.saveFollowee({
            user:"Alice", followee:"Charlie"
        })
    ...
    test("saveFollowee() should save a new followee where " +
        "there was no followees before", async () => {
        const followeeRepository
            = new FileSystemFolloweeFsRepository(testFolloweesPath);
        await fs.promises.writeFile(testFolloweesPath,JSON.stringify({
            Bob: ["Charlie"]
